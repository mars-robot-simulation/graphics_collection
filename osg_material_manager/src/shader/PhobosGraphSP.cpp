...
namespace osg_material_manager
{
    ...
    void PhobosGraphSP::parseGraph()
    {
        ...
        string actualPath = "";
        for (vit = model["sort"].begin(); vit != model["sort"].end(); ++vit)
        { // iterate over sorted node names
            string nodeName = vit.base()->getString();
            string nodeType = model["nodes"][nodeName]["type"].getString();
            if (nodeType == "uniform")
            {
                if (model["nodes"][nodeName]["uniform_builtin"].getString() == "false")
                { // TODO: Should actually get/read a bool right away..
